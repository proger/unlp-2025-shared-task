...
def prepare_data(X, y, label):
    y_bin = np.array([1 if label in (labels or []) else 0 for labels in y])
    return train_test_split(X, y_bin, test_size=0.2, random_state=42)

def run_logistic_regression(X, y, label, epochs=1000, lr=1):
    print(f"Training for label: {label}")
    X_train, X_test, y_train, y_test = prepare_data(X, y, label)
...
if __name__ == "__main__":
    ...
    for label in labels_list:
        results[label] = run_logistic_regression(X, y, label)

    with open("exp/cv_logistic.json", "w") as f:
